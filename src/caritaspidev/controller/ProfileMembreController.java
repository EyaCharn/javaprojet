...
import caritaspidev.services.ServiceRandomMailConfirmation;
...
public class ProfileMembreController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       // Compte CompteUserConnecter = myServices.chercherUtilisateurByUsernameDansLecompte(UserConneter.getUsername());
        NomPrenom.setText(UserConneter.getFirstname() + "+" + UserConneter.getLastname());
        //System.out.println("CompteUserConnecter" + CompteUserConnecter.toString());
        //   user.setText(String.valueOf(UserConneter.getId()));
        if (UserConneter.getFirstname() != null) {
            labelNom.setText(UserConneter.getFirstname());
        }
        if (UserConneter.getLastname() != null) {
            labelPrenom.setText(UserConneter.getLastname());
        }
     ...
     labelEmail.setText(UserConneter.getEmail());
        ...
        if (UserConneter.getRoles() != null) {
            ...
            } else if (role.equals("a:1:{i:0;s:17:\"ROLE_volontaire\";}")) {

                labelRole.setText("Volontaire");

            } 
            else if (role.equals("a:1:{i:0;s:17:\"ROLE_Refugie\";}")) {

                labelRole.setText("Réfugié");
    ...
    private void informationPersonnelleClick(MouseEvent event) {
        ...
        nom.setText(UserConneter.getFirstname());
        prenom.setText(UserConneter.getLastname());

        String ROLEBASE = "";
        if (UserConneter.getRoles().equals("a:1:{i:0;s:11:\"ROLE_volontaire\";}")) {
            ROLEBASE = "Volontaire";
        } else if (UserConneter.getRoles().equals("a:1:{i:0;s:10:\"ROLE_ADMIN\";}")) {
            ROLEBASE = "Admin";
        }
         else if (UserConneter.getRoles().equals("a:1:{i:0;s:10:\"ROLE_Refugie\";}")) {
            ROLEBASE = "Réfugié";
        }
        else {
            ROLEBASE = "Medecin";
    ...
    private void EnregistrerInfoPersonnelle(MouseEvent event) throws IOException {
        ...
        if ("".equals(nom.getText())) {
            ...
            user user = new user(  username.getText().trim(),nom.getText().trim(), prenom.getText().trim(),
                    ROLEBASE );
            System.out.println(user.toString());
            myServices.modifierUtilisateursInformationPersonnel(user, Integer.parseInt(IdUser.getText().trim()));

            nom.setText(myServices.chercherUtilisateurByid(Integer.parseInt(IdUser.getText().trim())).getFirstname() + "  " + myServices.chercherUtilisateurByid(Integer.parseInt(IdUser.getText().trim())).getLastname());
...
//            }
            ...
            showInformationPersonnelle.setVisible(false);
            ImporterImage.setVisible(false);
            setNode(FXMLLoader.load(getClass().getResource("/pidev/bonplan/GUI/ProfileMembre.fxml")));
...
//•	Contains at least 1 special character from the set: !@#$%^&*
    ...
    private void VerifPasswordActuelle(KeyEvent event) {
        ...
        if (event.getCode() == KeyCode.ENTER) {
              ...
              System.out.println("username"+UserConneter.getUsername());
